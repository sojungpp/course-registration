...
public class VSugangsincheon {
	...
	public void show(OHwewon oHwewon) {
		System.out.println("강좌검색(1), 수강내역확인(2), 뒤로가기(3)");
		...
		try {
			...
			for(OLecture lecture: lectures) {
				for(int i=0; i<10; i++) {
					...
					else if (lecture.getProfessor().equals(oLecture.getProfessor())) {System.out.println("강좌 시간이 같아 신청할 수 없습니다.");
					search(oHwewon);}
				}  
			}
			...
			fileWriter.flush();
			System.out.println("수강신청 완료됐습니다.");
			search(oHwewon);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void list(OHwewon oHwewon) {
		CLecture cLecture = new CLecture();
		Vector<OLecture> lectures = cLecture.getAllList(oHwewon.getId()+"S");
		if(lectures == null) {System.out.println("수강내역이 없습니다.");}
		System.out.println(oHwewon.getName()+"님의 수강내역");
		...
		System.out.println("강좌추가(1), 강좌삭제(2), 미리담기추가(3), 뒤로가기(4)");
			...
			try {
				...
				for(OLecture lecture2: lectures2) {
					...
					fileWriter.write(lecture.getId()+" "
								...
								+lecture.getProfessor()+"\r\n");
				fileWriter.flush();
				System.out.println("미리담기에 신청됐습니다.");
				list(oHwewon);
			} catch (IOException e) {
				e.printStackTrace();
				}	
